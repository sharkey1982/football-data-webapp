-- get_betting_returns: also reads match_predictions (past seasons, retro-fitted).
-- ============================================================================
...
begin
  ...
  with played as materialized (
    ...
    union all
    -- Past seasons: archived matches predicted from point-in-time (retro-)fits.
    -- Skipped where a fixture prediction already covers the same match.
    select mp.predicted_home_goals::numeric, mp.predicted_away_goals::numeric,
           coalesce(mfr2.rho, 0)::numeric, m2.match_id,
