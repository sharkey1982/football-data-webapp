-- Individual bets behind Model Returns. get_betting_returns becomes a summary
-- of this, so the bet list and the totals can never disagree.
--
-- Built first with the previous price logic unchanged and proven identical
-- to the old get_betting_returns (48 settings, 120 rows, 6,238 bets), then
-- the "median" basis fixed: match_odds.bookmaker includes football-data's
-- 'Max' and 'Avg' MARKET SUMMARIES, which are not bookmakers, so the old
-- median mixed them with two or three real books. It is now the 'Avg' row.
-- "Best" (highest price on file) is unchanged -- 'Max' is not always highest.
create or replace function public.get_betting_bets(
...
returns table(match_id bigint, match_date date, home_team text, away_team text,
  home_goals integer, away_goals integer, selection text, model_p numeric, price numeric,
  edge numeric, won boolean, stake numeric, profit numeric, prices jsonb,
...
grant execute on function public.get_betting_bets(numeric,text,boolean,boolean,numeric,bigint) to anon, authenticated;

create or replace function public.get_betting_returns(
  p_edge numeric default 0.05, p_market text default '1x2', p_closing boolean default true,
  p_best_price boolean default true, p_stake numeric default 10, p_season_id bigint default 13)
returns table(selection text, bets integer, staked numeric, returned numeric, profit numeric,
...
as $$
  ...
  from public.get_betting_bets(p_edge, p_market, p_closing, p_best_price, p_stake, p_season_id) b
