-- Country Insights (/football/countries-compared): one row per country's
-- top flight per season.
--
-- Top flight = tier 1, plus E0 by code: the English leagues were created
-- before `tier` was populated and still carry null.
--
-- football-data.co.uk's all-seasons "extra league" files (source_file
-- .../new/XXX.csv) carry no cards or half-time scores. matches stores cards
-- NOT NULL default 0, so those rows are left out of the card averages (null
-- when a league has none) instead of reading as zero; half-time results are
-- already null there, so the comeback rate is null too.
create or replace function public.get_country_league_summary()
returns table(country_id bigint, country_name text, league_code text, league_name text, season_label text,
  matches bigint, goals_per_game numeric, home_goals_per_game numeric, away_goals_per_game numeric,
  home_win_pct numeric, draw_pct numeric, away_win_pct numeric, yellows_per_game numeric, reds_per_game numeric,
  over_two_five_pct numeric, both_scored_pct numeric, nil_nil_pct numeric, comeback_pct numeric)
...
$function$;
comment on function public.get_country_league_summary() is
  'Country Insights page: per top-flight league per season, goals/results/cards averages. Cards and half-time comebacks are null for leagues from football-data.co.uk extra (all-seasons) files, which carry neither.';
grant execute on function public.get_country_league_summary() to anon, authenticated;
