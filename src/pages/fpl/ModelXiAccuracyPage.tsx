      ...
      {weeks.map((w) => (
        <div key={w.fpl_event_id}>
          <div className="flex items-baseline justify-between text-xs text-ink-500 mb-1">
            <span className="font-medium text-ink-700">Gameweek {w.fpl_event_id}</span>
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="w-10 text-right text-xs font-mono text-ink-700">{w.model_xi_actual_points}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-24 shrink-0 text-[11px] text-ink-500">Its forecast</span>
              <div className="flex-1 bg-chalk-100 rounded h-4 overflow-hidden">
                <div className="h-full bg-cup-600" style={{ width: pct(w.model_xi_projected) }} />
              </div>
              <span className="w-10 text-right text-xs font-mono text-ink-500">{w.model_xi_projected.toFixed(0)}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-24 shrink-0 text-[11px] text-ink-500">Perfect XI</span>
              <div className="flex-1 bg-chalk-100 rounded h-4 overflow-hidden">
                <div className="h-full bg-chalk-300" style={{ width: pct(w.actual_xi_points) }} />
              </div>
              <span className="w-10 text-right text-xs font-mono text-ink-500">{w.actual_xi_points}</span>
            </div>
          </div>
        </div>
      ))}
  ...
  return (
    <div className="bg-chalk-50 border-t border-chalk-300 px-3 py-3">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <h3 className="text-xs font-medium text-ink-700 mb-1">
            The model&rsquo;s XI <span className="text-ink-500">({model.reduce((t, p) => t + p.actual_points, 0)} pts)</span>
          </h3>
          <ul className="space-y-0.5">
            {model.map((p) => (
              <li key={p.fpl_player_id} className="flex items-baseline gap-2 text-xs">
                <span className="w-8 shrink-0 text-ink-500">{p.position_label}</span>
                <span className={`flex-1 ${p.in_perfect_xi ? 'text-pitch-800 font-medium' : 'text-ink-700'}`}>
                  {p.in_perfect_xi && '✓ '}
                  {p.web_name}
                  <span className="text-ink-500"> · {p.team_name}</span>
                </span>
          ...
          <p className="text-[11px] text-ink-500 mt-1">projected · actual</p>
        </div>
        <div>
          <h3 className="text-xs font-medium text-ink-700 mb-1">
            The week&rsquo;s best XI <span className="text-ink-500">({actual.reduce((t, p) => t + p.points, 0)} pts)</span>
          </h3>
          <ul className="space-y-0.5">
            {actual.map((p) => (
              <li key={p.fpl_player_id} className="flex items-baseline gap-2 text-xs">
                <span className="w-8 shrink-0 text-ink-500">{p.position_label}</span>
                <span className={`flex-1 ${modelIds.has(p.fpl_player_id) ? 'text-pitch-800 font-medium' : 'text-ink-700'}`}>
          ...
          <p className="text-[11px] text-ink-500 mt-1">chosen with hindsight</p>
        </div>
      </div>
    </div>
  );
      ...
      {weeks !== null && summary !== null && weeks.length > 0 && (
        <>
          {summary.forecastWeeks.length === 0 && (
            <div className="border border-amber-400 bg-amber-50 rounded-lg p-3 text-sm text-ink-700">
              ...
              <tbody>
                {weeks.map((w) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right font-mono text-ink-500">{w.model_xi_projected.toFixed(0)}</td>
                    <td className="px-3 py-2 text-right font-mono text-ink-500">{w.actual_xi_points}</td>
                    <td className="px-3 py-2 text-right font-mono text-ink-500 hidden sm:table-cell">{w.overlap_count}</td>
                    <td className="px-3 py-2 text-xs">
                      ...
                      <span className="text-ink-500"> · {w.players_projected} players</span>
