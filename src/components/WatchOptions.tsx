...
const isRuleRow = (o: WatchOffer) => (o.source ?? '').startsWith('rule:');
...
export default function WatchOptions({ offers, partners, fixtureId, page }: Props) {
  ...
  // Rows generated by a rule (e.g. the Saturday 3pm blackout) have no
  // listing behind them: show the reason, not a source name or a
  // "verified" date.
  const provenance = (offers ?? [])
    .map((o) => ({ o, rule: isRuleRow(o), v: isRuleRow(o) ? null : verification(o.verifiedAt) }))
  ...
  return (
    <div>
      ...
      {provenance.length > 0 && provenance.every((p) => p.rule) && (
        <p className="mt-1 text-[11px] text-ink-500">{provenance.map((p) => p.o.availabilityNotes).filter(Boolean).join(' ')}</p>
      )}
      {provenance.length > 0 && !provenance.every((p) => p.rule) && (
        <details className="mt-1 text-[11px] text-ink-500">
          ...
          <ul className="mt-1 space-y-0.5">
            {provenance.map(({ o, v, rule }) => rule ? (
              <li key={o.broadcastId}>{o.availabilityNotes}</li>
            ) : (
