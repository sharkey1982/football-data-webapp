...
for(const b of plan){
  if(b==='match'){
    ...
    if(gw>1)add(`Gameweek ${gw} — match-day summary: position, cash, team health, expected goals, clean sheets; PAY DAY (wages out)`);
    add(`Gameweek ${gw} — preview v ${opp}: them v you; ${PRE[k]} (every option: xG both ways, clean sheet, win/draw/lose); Kick off; the pitch`);
